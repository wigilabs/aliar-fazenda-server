...
class NewRelicProvider extends ServiceProvider {
  ...
  boot() {
     ...
     */
    newrelic.instrumentWebframework('../src/Server', function (shim, Server) {
      ...
      shim.wrap(Server.prototype, ['registerGlobal', 'registerNamed', 'use'], function (shim, originalFn) {
  ...
  wrapRoute(shim, route, handler) {
    ...
    if (shim.isFunction(handler)) {
      return shim.recordMiddleware(handler, {
        ...
        route,
        req(shim, fn, fnName, args) { return args[0].req; },
        res(shim, fn, fnName, args) { return args[0].res; },
        next: shim.SECOND,
        params(shim, fn, fnName, args) { return args[0].params; }
      });
